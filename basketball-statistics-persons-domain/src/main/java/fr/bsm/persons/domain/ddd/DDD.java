package fr.bsm.persons.domain.ddd;

import static fr.bsm.persons.domain.ddd.DDD.Layer.DOMAIN;
import static fr.bsm.persons.domain.ddd.DDD.Layer.INFRASTRUCTURE;
import static fr.bsm.persons.domain.ddd.DDD.Layer.APPLICATION;
...
@DDD.RefCardDescription("58: Design must be model-first, not data-first or UI-first\n"
...
public @interface DDD {
	...
	@Retention(RUNTIME)
	@Target(ANNOTATION_TYPE)
	@RefCardDescription("7: Apart from the exposition layer, the top-level modules should be Application, Domain, and Infrastructure\n"
			...
			+ "This recommendation concerns the horizontal layering by responsibility, it doesn’t preclude deployment units based on vertical slices of functionality. In that case the same Application/Domain/Infrastructure layers should be used within each functional deployment unit.\n"
			+ "This recommendation doesn’t concern the granularity of build artifacts.\n" +
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("17: Avoid the primitive obsession anti-pattern with Value Objects\n"
			+ "Designs that use too many low-level types such as primitives or strings are difficult to understand because they’re weakly typed and repetitive. This common anti-pattern is called “primitive obsession” and should be avoided. \n"
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("11: The names of Entities must be nouns of the Ubiquitous language\n"
			...
			+ "Entities are defined by the continuity of their identity, and not by their identity. The identifier field(s) used to represent their identity must be unique within the Entity’s Bounded Context.\n"
			+
	...
	@Target({ TYPE, FIELD })
	@Module(DOMAIN)
	@RefCardDescription("16: Entities should use specific Value Object Identifiers (VOIDs)\n"
	...
	@Target(PACKAGE)
	@Module(DOMAIN)
	@RefCardDescription("45: Distinguish eventual invariants from transactional invariants, define the inconsistency window for eventual invariants\n"
			...
			+ "Therefore, the boundaries of Aggregates must be chosen to allow transactional invariants: they must include all Entities that participate to the Aggregate’s invariant(s).\n"
			+
			...
			+ "If creating an aggregate becomes a complex operation, encapsulate this creation inside a Factory. The Factory’s contract guarantees that it produces a completely built aggregate that satisfies all its invariants.\n")
	@interface Aggregate {
	}
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("49: The Aggregate root must be the unique entry point into the Aggregate \n"
			+ "The safest way to enforce the Aggregate’s invariants is to only allow updates through the root. For example, the Aggregate Root can increment the Optimistic Locking version field every time it modifies one of its Entities.\n"
			...
			+ "Other components may reference only the Aggregate Root. They may access the non-root Entities with restrictions (only in read-only mode, and only in a transient way –storing a reference is not allowed-). The preferred approach is to encapsulate query behaviour in the Aggregate Root. \n")
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("36: Behaviour that doesn’t fit naturally in an Entity should be placed in a Domain Service\n"
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("52: Track significant transitions of domain state with Domain Events \n"
			+ "Domain Events enable non-intrusive audit trails (traceability, compliance), operational tracking for later analysis (including feedback loops to operational processes), and integration with external systems…\n"
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("37: Repositories should be used to represent access to persistent collections of elements\n"
			+ "From the point of view of the Domain layer, Repositories simulate in-memory collections of elements. This enables powerful behaviour in the Domain layer. Since the Repositories implementations are in the Infrastructure layer (Separated Interface), this doesn’t cause infrastructure dependencies in the Domain layer.\n"
			+
			...
			+ "When applicable a “smart plural” can be more domain-specific, for instance Library for a Repository of Books.\n")
	...
	@Target(TYPE)
	@Module(DOMAIN)
	@RefCardDescription("44: Within the Domain layer, software contingencies must be represented by Infrastructure Services \n"
			...
			+ "Examples of contingencies include: integration, mapping, external communication, …\n")
	...
	@Target(TYPE)
	@Module(APPLICATION)
	@RefCardDescription("26: Use-case logic  must not be implemented in the Exposition layer\n"
			...
			+ "If an Application Service includes several related use cases, this naming applies to its methods (eg: SearchBooks, AddBookToWishList). In that case, one possible naming for the Application Service’s class is to use the Actions suffix (ex: BookActions).\n"
			...
			+ "Unlike other kinds of services, Application Services are not “self-composable”. Shared logic must be placed into the Domain layer instead.\n"
			+

			"34: An Application Service should modify a single aggregate instance\n"
			+ "Only a single aggregate instance should be modified within a transaction, and the transaction’s scope is the Application.\n"
			...
			+ "When a view needs data from several Aggregates, this set of data is specific to the use case. It is thus the Application Service’s responsibility to assemble this DTO; it can delegate it to a DTO assembler.\n")
